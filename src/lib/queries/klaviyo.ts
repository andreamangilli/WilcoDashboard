...
export const getKlaviyoOverview = unstable_cache(
  async () => {
    ...
    const { data } = await supabase
      .from('klaviyo_campaigns')
      .select('recipients, opens, clicks, conversions, revenue, unsubscribes, channel')
      .eq('status', 'Sent');

    const email = { recipients: 0, opens: 0, clicks: 0, conversions: 0, revenue: 0, unsubscribes: 0, count: 0 };
    ...
    for (const row of data || []) {
      if (row.channel === 'sms') {
        ...
        email.recipients += row.recipients || 0;
    ...
    return {
      email: {
        ...email,
        open_rate: email.recipients > 0 ? email.opens / email.recipients : 0,
        click_rate: email.recipients > 0 ? email.clicks / email.recipients : 0,
      },
      sms,
      total: {
